...
export function TopBar({
  ...
  return (
    <header className="h-14 bg-paper border-b border-line flex items-center px-6 gap-4">
      ...
      <button
        type="button"
        className="relative h-9 px-3 rounded inline-flex items-center gap-2 text-slate-dark hover:bg-cream-2 transition-colors group"
        title={`${pendientes} acciones pendientes`}
      >
        <Bell className="w-[18px] h-[18px]" strokeWidth={1.5} />
        <span className="text-[12px] font-medium text-slate-dark group-hover:text-ink transition-colors">
          Pendientes
        </span>
        {pendientes > 0 && (
