...
const emptyDraft = (): DraftLine => ({ label: '', amount: '', recuperoTypeCode: '' })

export function RecurringChargesEditor({ contractId, currentRent }: Props) {
  ...
  function patchRow(id: string, patch: Partial<{ label: string; amount: number; recuperoTypeCode: string | null; active: boolean }>) {
    ...
    <div className="bg-paper border border-line rounded p-4">
      <div className="flex items-baseline justify-between gap-3 flex-wrap mb-3">
        <div>
          ...
          <p className="text-[12px] text-slate mt-0.5 max-w-[540px]">
            Cargos fijos que se suman al alquiler cada mes (ABL, THU, Camuzzi, etc.).
            Aparecen en la columna <strong className="text-ink">Recargos</strong> de la
            planilla con un puntito verde cuando el cobro está cargado o rojo cuando falta.
          </p>
        </div>
        {charges.length > 0 && (
        ...
        <table className="w-full text-[12.5px] border-collapse min-w-[600px]">
          <thead className="bg-gray-50 text-[10px] uppercase tracking-wider text-gray-600">
            <tr className="border-b border-gray-200">
              <th className="text-left  px-2 py-1.5 font-medium w-[150px]">Etiqueta</th>
              ...
              <th className="text-center px-2 py-1.5 font-medium w-[70px]">Activo</th>
              <th className="w-8"></th>
            </tr>
          </thead>
          <tbody>
            {!loaded && (
              <tr><td colSpan={5} className="p-3 text-center text-gray-500 italic">Cargando…</td></tr>
            )}
            {loaded && charges.length === 0 && (
              <tr><td colSpan={5} className="p-3 text-center text-gray-500 italic">Sin recargos cargados.</td></tr>
            )}
            {charges.map(r => (
      ...
      <div className="mt-3 bg-cream/40 border border-gray-200 rounded p-2">
        ...
        <div className="grid grid-cols-[150px_120px_1fr_auto] gap-2 items-center">
          <input
            type="text"
            list="recurring-charge-labels"
            value={draft.label}
