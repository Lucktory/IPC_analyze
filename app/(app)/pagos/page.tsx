...
const filterCounts = {
  all: pagos.length,
  IN:  pagos.filter(p => p.direccion === 'IN').length,
  OUT: pagos.filter(p => p.direccion === 'OUT').length,
}
...
export default function PagosPage() {
  ...
  const visiblePagos = filter === 'all' ? pagos : pagos.filter(p => p.direccion === filter)

  const tabClass = (active: boolean) =>
    'px-3 py-1.5 rounded-sm text-[12px] font-medium transition-colors ' +
    (active
      ? 'border border-line bg-cream-2 text-ink'
      : 'border border-transparent text-slate-dark hover:text-ink hover:bg-cream-2/60')
        ...
        <div className="px-5 py-4 border-b border-line flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-[12px] text-slate mt-0.5">
              {visiblePagos.length} {visiblePagos.length === 1 ? 'movimiento' : 'movimientos'}
              {filter !== 'all' && ` · filtrado por ${filter === 'IN' ? 'ingresos' : 'egresos'}`}
            </p>
          </div>
          <div className="flex gap-1.5" role="tablist" aria-label="Filtrar movimientos">
            <button
              type="button"
              role="tab"
            ...
            >
              Todos <span className="tabular-nums text-slate ml-1">{filterCounts.all}</span>
            </button>
            <button
              type="button"
              role="tab"
              ...
              className={tabClass(filter === 'IN')}
            >
              Ingresos <span className="tabular-nums text-slate ml-1">{filterCounts.IN}</span>
            </button>
            <button
              type="button"
              role="tab"
              ...
              className={tabClass(filter === 'OUT')}
            >
              Egresos <span className="tabular-nums text-slate ml-1">{filterCounts.OUT}</span>
            </button>
          </div>
        </div>
        ...
        <table className="w-full text-[13px] min-w-[820px]">
          ...
          <tbody>
            {visiblePagos.length === 0 ? (
              ...
              visiblePagos.map((p, i) => (
                <tr
                  key={`${p.fecha}-${p.contrato}-${i}`}
                  className={
                    p.estado === 'Atrasado'
